...
class AmpacheRemoteClient(object):

    def __init__(self, hostname, port, username, password, ssl, context):
        super(AmpacheRemoteClient, self).__init__()

        if not (hostname and port and username and password):
            logger.error('Ampache API settings are not fully defined: %s %s %s %s %s' % (hostname, port, username, password, ssl))
        else:
            ...
            if context:
                self.api_context = context + "/server"
            else:
                self.api_context = "/server"

            self.api = libmpache.Connection(self.api_hostname, self.api_user, self.api_pass, port=int(self.api_port), serverPath=self.api_context)
            logger.info('Connecting to Ampache library %s:%s as user %s', self.api_hostname, self.api_port, self.api_user)
            try:
                self.api.ping()
            except Exception as e:
                logger.error('Ampache Authentication error: %s' % e)
                exit()
